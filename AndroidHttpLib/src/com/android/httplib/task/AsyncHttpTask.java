...
public abstract class AsyncHttpTask<Params, Progress, Result> {
	...
	public AsyncHttpTask() {
		...
		if (mRemoteWorkExecutor == null) {
			mRemoteWorkExecutor = new ThreadPoolExecutor(CORE_POOL_SIZE, MAXIMUM_POOL_SIZE, KEEP_ALIVE, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(10),
					new ThreadFactory() {
						private final AtomicInteger mCount = new AtomicInteger(1);

						public Thread newThread(Runnable r) {
							return new Thread(r, "AsyncHttpTask remote#" + mCount.getAndIncrement());
						}
					});
		}
		...
		if (mLocalWorkExecutor == null) {
			mLocalWorkExecutor = new ThreadPoolExecutor(1, MAXIMUM_POOL_SIZE, 0, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(10), new ThreadFactory() {
				private final AtomicInteger mCount = new AtomicInteger(1);

				public Thread newThread(Runnable r) {
					return new Thread(r, "AsyncHttpTask local#" + mCount.getAndIncrement());
				}
			});
	...
	protected final AsyncHttpTask<Params, Progress, Result> executeRemote(Params... params) {
		return executeOnExecutor(mRemoteWorkExecutor, params);
	}

	protected final AsyncHttpTask<Params, Progress, Result> executeLocal(Params... params) {
	...
	protected final AsyncHttpTask<Params, Progress, Result> executeOnExecutor(Executor exec, Params... params) {

		if (exec == null)// thread pool shutdowned
			return this;

		if (LogHttp.isDevelopMode()) {

			ThreadPoolExecutor tpe = (ThreadPoolExecutor) exec;
			LogHttp.w(TAG, "async http task getActiveCount=" + tpe.getActiveCount() + ", getTaskCount = " + tpe.getTaskCount());
		}
